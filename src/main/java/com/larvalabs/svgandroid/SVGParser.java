...
public class SVGParser {
  ...
  private static void drawArc(Path p,
    ...
    final float R = (float) Math.sqrt((rxs * rys - rxs * y1ts - rys * x1ts) / (rxs * y1ts + rys * x1ts)) * ((largeArc
        == sweepArc) ? -1 : 1);
  ...
  static class SVGHandler extends DefaultHandler {
    ...
    private Gradient doGradient(boolean isLinear, Attributes atts) {
      ...
      gradient.tilemode = (spreadMethod.equals("reflect"))
    ...
    @Override public void startElement(String namespaceURI, String localName, String qName, Attributes atts)
        throws SAXException {
        ...
        if (viewboxStr != null) {
          ...
          if (dims.length == 4) {
            ...
            if (x1 != null && x2 != null && y1 != null && y2 != null) {
              ...
              canvas.drawColor(Color.parseColor(SVG_FILL));
     ...
     * See <a href="http://stackoverflow.com/questions/4567636/java-sax-parser-split-calls-to-characters">here</a> for
     * details.
     */
    protected String getFullText() {
      if (textBuilder == null) {
        throw new IllegalStateException(
            "Must be called only if current element is 'text' and when the value is fully parsed!");
      }
